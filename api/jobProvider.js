...
export const jobProviderAPI = () => {
  ...
  const getJob = async (id_job) => {
    return await getRequest(
      id_job ? "jobprovider/job/" + id_job : "jobprovider/job",
      true
    );
  };
  const postJob = async (body, id_job) => {
    return (
      (await jobPostValidator(body)) &&
      postRequest(
        id_job ? "jobprovider/job/" + id_job : "jobprovider/job",
        body
      )
  ...
  const putJob = async (body, id_job) => {
    return (
      (await jobPostValidator(body)) &&
      putRequest("jobprovider/job/" + id_job, body)
    );
  };
  ...
  const postPretest = async (body, id_pretest) => {
    return await postRequest(
      id_pretest ? "jobprovider/modul/" + id_pretest : "jobprovider/modul/",
      body
    );
  };
  ...
  const getModule = async (id_module) => {
    return await getRequest(
      id_module ? "jobprovider/pretest/" + id_module : "jobprovider/pretest",
      true
    );
  };
  ...
  const getListJobOpening = async (sortBy, status, id_job) => {
    return await postRequest(
      id_job ? "jobprovider/listjob/" + id_job : "jobprovider/listjob/",
  ...
  // Tahap 1
  const getDetailAnswerAPI = async (
    job_seeker_id,
    job_post_id,
    pretest_modul_detail_id
  ) => {
    return await getRequest(
      `jobprovider/detailanswer/` +
        job_seeker_id +
        "/" +
        job_post_id +
        "/" +
  ...
  const getFileAnswer = async (apply_job_answer_id) => {
    return await getRequest(
      `jobprovider/selection/file_answer/` + apply_job_answer_id,
      true,
      "arraybuffer"
    );
  };
  const putInputScore = async (body, pretest_modul_detail_id) => {
    return await putRequest(
      `jobprovider/scoringanswer/` + pretest_modul_detail_id,
      body
    );
  };
  const postFinishScoring = async (body, pretest_modul_detail_id) => {
    return await postRequest(
      `jobprovider/finishscoringanswer/` + pretest_modul_detail_id,
      body
    );
  };
  // Tahap 2
  const getDetailJobSeeker = async (job_seeker_id) => {
    return await getRequest(
      `jobprovider/selection/user_detail/` + job_seeker_id
    );
  };
  // Tahap 3
  const getSchedule = async (job_post_id, month, year) => {
    return await getRequest(
      "jobprovider/selection/scheduling/" +
        job_post_id +
        "/" +
        month +
        "/" +
  ...
  const postBlastSchedule = async (body, job_post_id) => {
    return await postRequest(
      "jobprovider/blastinschedule/" + job_post_id,
      body
    );
  };
  // Tahap 4
  const getFormGrade = async (job_seeker_id, job_post_id) => {
    return await getRequest(
      "jobprovider/selection/grade/" + job_seeker_id + "/" + job_post_id
    );
  };
  const getResultGrade = async (job_seeker_id, job_post_id) => {
    return await getRequest(
      "jobprovider/selection/gradedetail/" + job_seeker_id + "/" + job_post_id
    );
  };
  ...
  const postLinkInterview = async (body, job_post_id) => {
    return await postRequest(
      "jobprovider/selection/savelink/" + job_post_id,
      body
    );
  };
  const putAbstain = async (job_seeker_id, job_post_id) => {
    return await putRequest(
      "jobprovider/selection/abstain/" + job_seeker_id + "/" + job_post_id
    );
  };
  // Tahap 5
  const postFinishSelection = async (body, job_post_id) => {
    return await postRequest(
      "jobprovider/selection/finish/" + job_post_id,
      body,
      true
    );
  };
  ...
  const getEmployee = async (page, file, file_status, employee_status) => {
    let param = "?";
    if (file != null && file != "Semua") {
      param = param + "file=" + file + "&";
    }
    if (file_status != null && file_status != "Semua") {
      param = param + "file_status=" + file_status + "&";
    }
    if (employee_status != null && employee_status != "Semua") {
      param = param + "employee_status=" + employee_status;
    }
  ...
  const getEmployeeDataPDF = async (employee_id) => {
    return await getRequest(
      "jobprovider/pdf/employee/" + employee_id,
      true,
      "arraybuffer"
    );
  };
  ...
  const getRemuneration = async (remuneration_id, employee_name) => {
    ...
    if (employee_name != null) {
      param = param + "employee_name=" + employee_name;
    }
  ...
  const getRemunerationDetail = async (remuneration_detail_id) => {
    return await getRequest(
      "jobprovider/remuneration/detail/" + remuneration_detail_id
    );
  };
  ...
  const getListEmployee = async (remuneration_id, employee_name) => {
    ...
    if (remuneration_id != null) {
      param = param + "batch_remuneration_id=" + remuneration_id;
    }
  ...
  const putDetailRemuneration = async (body, remuneration_detail_id) => {
    return await putRequest(
      "jobprovider/remuneration/employee/" + remuneration_detail_id,
      body
    );
  };
  const putRemunerationRequestType = async (body, remuneration_detail_id) => {
    return await putRequest(
      "jobprovider/remuneration/request_type/" + remuneration_detail_id,
      body
    );
  };
  const deleteListEmployee = async (employee_id) => {
    return await deleteRequest(
      "jobprovider/remuneration/employee/" + employee_id
    );
  };
  const getAttachment = async (remuneration_id) => {
    return await getRequest(
      "jobprovider/remuneration/attachment/" + remuneration_id
    );
  };
  const postAttachment = async (body, attachment_id) => {
    return await postRequest(
      "jobprovider/remuneration/attachment/" + attachment_id,
      body
    );
  };
  ...
  const getDownloadSimulation = async (remuneration_id) => {
    return await getRequest(
      "jobprovider/downloadsimulation/" + remuneration_id,
      true,
      "arraybuffer"
    );
  };
  const postRequestApproval = async (remuneration_id) => {
    return await postRequest(
      "jobprovider/remuneration/request/" + remuneration_id
    );
  };
  const postRemunerationRecreate = async (remuneration_id) => {
    return await postRequest(
      "jobprovider/remuneration/recreate/" + remuneration_id
    );
  };
  ...
  const getMessageAttachment = async (detail_message_id) => {
    return await getRequest(
      $auth.user.role_user == "jobprovider"
        ...
        : "jobseeker/inbox/download_file/" + detail_message_id,
      true,
      "arraybuffer"
    );
  };
  ...
  const getDownloadTemplate = async () => {
    return await getRequest(
      "jobprovider/nonselection/template_download",
      true,
      "arraybuffer"
    );
  };
  ...
  const postCreateQuestion = async (body, job_post_id) => {
    return await postRequest(
      "jobprovider/nonselection/create_question/" + job_post_id,
      body
    );
  };
  ...
  // Payslip
  const getProviderPayslip = async (
    employee_id,
    year,
    month,
    limit,
    ...
    if (employee_id != null) {
      param = param + "employee_id=" + employee_id;
    }
  ...
  const getTalentMitra = async (params) => {
    try {
      const queryParams = new URLSearchParams({
        ...
        competence_test_id: params.competence_test_id || "",
      });

      return await getRequest(`jobprovider/talent/mitra?${queryParams}`);
    } catch (error) {
  ...
  const getJobPosts = async (params = {}) => {
    try {
      const response = await getRequest("/jobprovider/job_post");
  ...
  const inviteTalent = async (payload) => {
    try {
      return await postRequest("jobprovider/talent/invite", payload);
    } catch (error) {
      console.error("Error inviting talents to jobs:", error);
      throw error;
    }
  };
