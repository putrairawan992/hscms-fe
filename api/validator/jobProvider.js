...
export const jobProviderValidator = () => {
    ...
    const jobPostValidator = (body) => {
        ...
        if(body.start_date == "null 00:00:00"){ return $notifier.showMessage({ content: 'Periode pekerjaan belum terisi.', status: 'warning' }) }
        if(body.end_date == "null 00:00:00"){ return $notifier.showMessage({ content: 'Periode pekerjaan belum terisi.', status: 'warning' }) }
        ...
        if(body.education_id.length == 0){ return $notifier.showMessage({ content: 'Pendidikan belum terisi.', status: 'warning' }) }
        ...
        if(body.benefit_id.length == 0){ return $notifier.showMessage({ content: 'Keuntungan dari Perusahaan belum terisi.', status: 'warning' }) }
