        ...
        <div style="height: 676px; overflow: auto; overflow-x: hidden">
          <v-row class="mt-0">
            <v-col cols="6" class="pt-0">
              <div class="remunerasi-list-title">Existing</div>
              <v-checkbox
                v-for="(value, key) in sortedExistingEmployees"
                v-model="existing"
                :value="value.employee_id"
                hide-details
                class="input-checkbox mt-2"
                color="#ae445a"
              >
                <template v-slot:label>
                  <div class="checkbox-remunerasi-label">
            ...
            <v-col cols="6" class="pt-0">
              <div class="remunerasi-list-title">New Hiring</div>
              <v-checkbox
                v-for="(value, key) in sortedNewHiringEmployees"
                v-model="new_hiring"
                :value="value.employee_id"
                hide-details
                class="input-checkbox mt-2"
...
export default {
  ...
  computed: {
    ...
    // Add computed properties for sorted employees with proper capitalization
    sortedExistingEmployees() {
      if (!this.data || !this.data.existing) return [];
      return [...this.data.existing]
        .map((employee) => ({
          ...employee,
          // Create formatted_name property but keep the original for sorting
          formatted_name: this.formatName(employee.employee_name),
        }))
        .sort((a, b) => a.employee_name.localeCompare(b.employee_name));
    },
    sortedNewHiringEmployees() {
      if (!this.data || !this.data.new_hiring) return [];
      return [...this.data.new_hiring]
        .map((employee) => ({
          ...employee,
          // Create formatted_name property but keep the original for sorting
          formatted_name: this.formatName(employee.employee_name),
  ...
  methods: {
    async getData(employee_name) {
      ...
      if (storageIdRemuneration) {
        this.id_remuneration = storageIdRemuneration;
      }
      await this.getListEmployee(this.id_remuneration, employee_name).then(
        (result) => {
          if (result) {
            this.data = result;
          }
    ...
    async submit() {
      if (this.new_hiring !== null && this.existing !== null) {
        if (this.new_hiring.length > 0 || this.existing.length > 0) {
          ...
          await this.postPickEmployee(
            { employee_id: combinedArray },
            this.id_remuneration
          ).then((result) => {
            localStorage.setItem(
              "id_remuneration",
              result.data.batch_remuneration_id
            );
            this.new_hiring = [];
            this.existing = [];
            this.closeDialog();
    ...
    selectAll() {
      if (this.data.existing != null) {
        this.existing = this.data.existing.map(
          (employee) => employee.employee_id
        );
      }
      if (this.data.new_hiring != null) {
        this.new_hiring = this.data.new_hiring.map(
          (employee) => employee.employee_id
        );
      }
    },
    ...
    formatName(name) {
      ...
      // For names that start with a number, don't change them
      if (/^[0-9]/.test(name)) {
        return name;
      }

      // For normal names, capitalize the first letter and make the rest lowercase
