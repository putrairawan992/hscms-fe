      ...
      <div class="my-2" style="position: relative">
        <div class="blokade-parent ma-8 pt-10">
                ...
                <div class="round">
                  ...
                  <div class="bicheck1">
                    <img
                      class="vector-icon1"
                      alt=""
                      src="@/assets/svg/vector-register.svg"
                    />
                    <b class="b3">1</b>
                  </div>
                </div>
                ...
                <div class="round">
                  ...
                  <div class="bicheck2">
                    <img
                      class="vector-icon"
                      alt=""
                      src="@/assets/svg/rectangle-4137.svg"
                    />
                    <b class="b2">2</b>
                  </div>
                </div>
          ...
          <div class="my-10">
            ...
            <p class="ma-0">Pastikan isi dengan lengkap dan benar.</p>
          </div>
          ...
          <v-row align="start">
              ...
              <v-row>
                <v-col cols="4" xs="2" md="2" lg="2" xl="2" xxl="2">
                  <input
                    ...
                    type="file"
                    ref="fileInput1"
                    @change="onSelectFile($event, 'curriculum_vitae')"
                  />
                  <div class="pas-foto">
                    <button
                      class="plus cursor-pointer"
                      @click="!preview && $refs.fileInput1.click()"
                    >
                      <img
                        v-if="curriculum_vitae"
                        class="pluscirclefill-icon"
                        alt="CV"
                      ...
                      <img
                        ...
                        class="pluscirclefill-icon"
                        alt="CV"
                        src="@/assets/svg/pluscirclefill.svg"
                      />
                    </button>
                    <div class="foto">
                ...
                <v-col cols="4" xs="2" md="2" lg="2" xl="2" xxl="2">
                  ...
                  <div class="pas-foto">
                    <button
                      class="plus cursor-pointer"
                      @click="!preview && $refs.fileInput2.click()"
                    >
                      <img
                        v-if="photo"
                        class="pluscirclefill-icon"
                ...
                <v-col cols="4" xs="2" md="2" lg="2" xl="2" xxl="2">
                  <input
                    ...
                    type="file"
                    ref="fileInput3"
                    @change="onSelectFile($event, 'ktp')"
                  />
                  <div class="pas-foto">
                    <button
                      class="plus cursor-pointer"
                      @click="!preview && $refs.fileInput3.click()"
                    >
                      <img
                        v-if="ktp"
                        class="pluscirclefill-icon"
                ...
                <v-col cols="4" xs="2" md="2" lg="2" xl="2" xxl="2">
                  <input
                    ...
                    type="file"
                    ref="fileInput4"
                    @change="onSelectFile($event, 'kk')"
                  />
                  <div class="pas-foto">
                    <button
                      class="plus cursor-pointer"
                      @click="!preview && $refs.fileInput4.click()"
                    >
                      <img
                        v-if="kk"
                        class="pluscirclefill-icon"
                ...
                <v-col cols="4" xs="2" md="2" lg="2" xl="2" xxl="2">
                  <input
                    ...
                    type="file"
                    ref="fileInput5"
                    @change="onSelectFile($event, 'npwp')"
                  />
                  <div class="pas-foto">
                    <button
                      class="plus cursor-pointer"
                      @click="!preview && $refs.fileInput5.click()"
                    >
                      <img
                        v-if="npwp"
                        class="pluscirclefill-icon"
                ...
                <v-col cols="4" xs="2" md="2" lg="2" xl="2" xxl="2">
                  <input
                    ...
                    type="file"
                    ref="fileInput6"
                    @change="onSelectFile($event, 'bpjs')"
                  />
                  <div class="pas-foto">
                    <button
                      class="plus cursor-pointer"
                      @click="!preview && $refs.fileInput6.click()"
                    >
                      <img
                        v-if="bpjs"
                        class="pluscirclefill-icon"
            ...
            <v-col>
              <p class="disclaimer-seluruh-data">
                Disclaimer: Seluruh data milik pengguna tidak akan digunakan
                <br />
                untuk kepentingan lain selain untuk melamar pekerjaan di website
                ini
              </p>
            </v-col>
          </v-row>
...
export default {
  ...
  data: () => ({
    ...
    listDokumen: ["Sudah Punya", "Belum Punya"],
    ...
    optiondokumennpwp: null,
    optiondokumenbpjskes: null,
    optiondokumenbpjstk: null,
  }),
  watch: {
    nominal(to, from) {
      this.nominal = this.useConvertToMoneyView(to);
    },
    optiondokumennpwp(newVal, oldVal) {
      if (oldVal !== undefined && newVal === "Belum Punya") {
        this.npwp_number = "-";
      }
    },
    optiondokumenbpjskes(newVal, oldVal) {
      if (oldVal !== undefined && newVal === "Belum Punya") {
        this.bpjskes = "-";
      }
    },
    optiondokumenbpjstk(newVal, oldVal) {
      if (oldVal !== undefined && newVal === "Belum Punya") {
        this.bpjstik = "-";
      }
    },
  ...
  async mounted() {
    await this.getRegistrationData().then((result) => {
      this.masterJobSpecialist = result?.master_job_specialist
  ...
  methods: {
    ...
    async submit() {
      ...
      if (this.department.length > 0) {
        for (let index = 0; index < this.department.length; index++) {
          body.append(
            `department[${index}]`,
            this.department.length > 0 ? this.department[index] : null
          );
          body.append(
            `company_name[${index}]`,
            this.company_name.length > 0 ? this.company_name[index] : null
          );
          body.append(
            `employment_contract[${index}]`,
            this.employment_contract.length > 0
              ? this.employment_contract[index]
              : null
          );
          body.append(
            `start_working[${index}]`,
            this.start_working.length > 0 ? this.start_working[index] : null
          );
          body.append(
            `end_working[${index}]`,
            this.end_working.length > 0 ? this.end_working[index] : null
          );
          body.append(
            `location[${index}]`,
            this.location.length > 0 ? this.location[index] : null
          );
        }
      }
      // ======= education ==========
      if (this.institute_name.length > 0) {
        for (let index = 0; index < this.institute_name.length; index++) {
          body.append(
            ...
            this.institute_name.length > 0 ? this.institute_name[index] : null
          );
          body.append(
            `degree[${index}]`,
            this.degree.length > 0 ? this.degree[index] : null
          );
          body.append(
            `education_program[${index}]`,
            this.education_program.length > 0
              ? this.education_program[index]
              : null
          );
          body.append(
            `start_study[${index}]`,
            this.start_study.length > 0 ? this.start_study[index] : null
          );
          body.append(
            `end_study[${index}]`,
            this.end_study.length > 0 ? this.end_study[index] : null
          );
          body.append(
            `ipk[${index}]`,
            this.ipk.length > 0 ? this.ipk[index] : null
          );
        }
      }
      // ======= certificate =========
      if (this.certificate_name.length > 0) {
        for (let index = 0; index < this.certificate_name.length; index++) {
          body.append(
            `certificate_name[${index}]`,
            this.certificate_name.length > 0
              ? this.certificate_name[index]
              : null
          );
          body.append(
            `organizer[${index}]`,
            this.organizer.length > 0 ? this.organizer[index] : null
          );
          body.append(
            `scores[${index}]`,
            this.scores.length > 0 ? this.scores[index] : null
          );
          body.append(
            `certificate_year[${index}]`,
            this.certificate_year.length > 0
      ...
      await this.postRegistration(body, this.id_registration).then(
        async (result) => {
          if (result) {
            this.preview = true;
            ...
            return this.$notifier.showMessage({
              content: "Mohon koreksi data anda kembali.",
              status: "success",
            });
          }
        }
      ...
      await this.$auth
        ...
        .then((result) => {
          this.$notifier.showMessage({
            content: "Anda berhasil Registrasi.",
            status: "success",
          });
    ...
    refreshForm(data) {
      ...
      this.date_birth = data.data_job_seeker
