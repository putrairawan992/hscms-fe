                            ...
                            <v-row align="center">
                                ...
                                <v-col cols="12" class="pt-0 input-checkbox-container">
                                    <multiselect
                                        ...
                                        placeholder="Pilih Karyawan" :allow-empty="false"
                                        class="header-select-input" label="employee_name"
                                        >
                                    </multiselect>
                                </v-col>
                            </v-row>
                        ...
                        <v-col v-for="value, key in data" cols="12" class="pb-0">
                            <div class="history-1">
                                <div class="frame-parent-draft">
                                    ...
                                    <div class="">{{ value.created_at }} WIB</div>
                                    <div class="edit-parent">
                                        <img @click="" class="action-icon" src="@/assets/svg/download-red.svg" />
                                    </div>
                                    <div class="edit-parent">
                                        <a class="edit-btn-container" :href="value.file" target="_blank" style="text-decoration: none;">
                                            <div class="attach-mpr-parent">
                                                <b class="button">Detail</b>
                                            </div>
                                        </a>
                                    </div>
                                </div>
...
export default {
    ...
    data: () => ({
        ...
        arrayTahun: ["2024", "2025"],
        arrayBulan: [
            { nama: "Januari", angka: 1, englishName: "January" },
            { nama: "Februari", angka: 2, englishName: "February" },
            { nama: "Maret", angka: 3, englishName: "March" },
            { nama: "April", angka: 4, englishName: "April" },
            { nama: "Mei", angka: 5, englishName: "May" },
            { nama: "Juni", angka: 6, englishName: "June" },
            { nama: "Juli", angka: 7, englishName: "July" },
            { nama: "Agustus", angka: 8, englishName: "August" },
            { nama: "September", angka: 9, englishName: "September" },
            { nama: "Oktober", angka: 10, englishName: "October" },
            { nama: "November", angka: 11, englishName: "November" },
            { nama: "Desember", angka: 12, englishName: "December" }
        ],
    }),
    watch: {
        ...
        bulanSelected(to, from) {
            this.month = to.englishName;
            this.getData();
        },
        employeeSelected(to, from) {
            this.employee_id = to.employee_id;
            this.getData();
            console.log('employeeSelected', to);
        },
    },
    setup() {
        const { getProviderPayslip, getMasterEmployee } = API()
        return { getProviderPayslip, getMasterEmployee };
    },
    async mounted() {
        this.getEmployee();
        this.getData();
    ...
    methods: {
        async getData(){
            await this.getProviderPayslip(this.employee_id, this.year, this.month, this.limit, this.paginate).then((result)=>{
                this.data = result.payslip_list;
            });
        },
        async getEmployee(){
            await this.getMasterEmployee().then((result)=>{
                console.log('this.employeeList', result);
